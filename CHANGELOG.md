...
- Experimental Claude runtime: explicit `claude:*` model ids served through the local `claude` CLI under its existing subscription sign-in. Text `/v1/chat/completions` and `/v1/completions` (stream and non-stream), stateless, loopback-only; unsupported routes, states, and parameters are rejected locally and explicitly (ADR 0004). Enabled by default; opt out with `[providers.claude].enabled = false` or `AIRELAYS_ENABLE_CLAUDE_EXPERIMENTAL=false`. `airelays doctor` includes Claude readiness checks.
